...
# (inspire de https://deptinfo-ensip.univ-poitiers.fr/ENS/doku/doku.php/tp:python:feux_foret)
#
#
...
# --------------------------------------------------------------------------------
# Renvoi le nombre de cellules en feu
def CountFeu():
    return sum((life.world[i][j]==FEU) for j in range(life.co) for i in range(life.li))
...
def Play():
    ...
    # affichage
    life.DrawWorld()
    # on regarde si il y a encore le feu
    if CountFeu() <= 0:
        flag = False
    if flag:
...
# --------------------------------------------------------------------------------
# Met le feu sur le clic gauche ...
def CellFeu(event):
    setCell(event, FEU)

# --------------------------------------------------------------------------------
# Met un arbre sur clic du milieu
def CellArbre(event):
    setCell(event, ARBRE)

# --------------------------------------------------------------------------------
# Vide la cellule sélectionnée sur clic droit de la souris
def CellVide(event):
    setCell(event, VIDE)
...
class Life(object):
    ...
    cells = { VIDE: color_VIDE, ARBRE: color_ARBRE, FEU: color_FEU, CENDRE: color_CENDRE }
    ...
    def init_world_raz(self):
        self.world = [[VIDE for j in range(self.co)] for i in range(self.li)]

    # initialise la grille avec des cellules aléatoires (vide ou forêt)
    def init_world_rnd(self):
        self.world = [[random.choice([VIDE, ARBRE]) for j in range(self.co)] for i in range(self.li)]
        ...
        if cell_type == VIDE:
            ...
            if cell_type == CENDRE:
                ...
                if cell_type == FEU:
                    ...
                    # pour un arbre:
                    # devient feu si un voisin est feu
                    # sinon reste arbre
                    # voisinage de Von Neuman (4 voisins, haut/bas/droite/gauche)
                    future = ARBRE
                    if (self.get(i-1, j) == FEU) or (self.get(i, j+1) == FEU) or (self.get(i+1, j) == FEU) or (self.get(i, j-1) == FEU):
                        future = FEU
        return future
...
if __name__ == "__main__":
    ...
    fen = TK.Tk()
    fen.title("Simulations de feux de forêt")
    ...
    canvas.bind("<Button-1>", CellFeu)
    canvas.bind("<Button-2>", CellArbre)
    canvas.bind("<Button-3>", CellVide)
